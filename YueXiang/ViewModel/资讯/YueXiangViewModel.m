...
@implementation YueXiangViewModel
...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask = [YueXiangNetManager getYueXiangInfoWithType:_type start:_start completionHandle:^(YueXiangModel *model, NSError *error) {
        if (_start == 0) {
...
- (YueXiangDataIndexpicModel *)modelForArr:(NSArray *)arr row:(NSInteger)row{
    return arr[row];
}
...
- (NSArray *)iconURLSForRowInList:(NSInteger)row{
    ...
    for (int i = 0; i < arr.count; i ++) {
        YueXiangDataIndexpicShowitemModel *model= arr[i];
